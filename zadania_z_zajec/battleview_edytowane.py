...
class BattleView(object):
    ...
    def __init__(self):
        ...
        self.time = 0.0
        self.dt = 0.0
        self.alreadyHit = False ## zmienna oznaczajaca czy czolg zostal juz trafiony podczas tego strzalu, domyslnie false

        self.shooting = False
        self.points = 0 # poczatkoa liczba punktow
    ...
    # !!! ta funkcja wykonuje sie co 100ms, wszystko w niej sprawdzam
    def on_update(self, time):
        if self.shooting:
            self.projectile.update(self.time)
            self.time += self.dt
            #sprawdzan pozycje pocisku i czy czolg zostal juz trafiony podczas tego strzalu
            if self.projectile.position[1] < 0.5 and abs(10 - self.projectile.position[0] < 0.5) and self.alreadyHit == False:
                self.alreadyHit = True #ustawiam ze czolg zostal juz trafiony podczas tego strzalu
                self.points = self.points + 1 #zwiekszam liczbe punktow
                print('%s %d' % ('Punkty:', self.points)) # wyswietlanie liczby punktow w konsoli
